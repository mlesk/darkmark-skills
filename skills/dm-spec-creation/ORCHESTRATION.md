...
4. **Detect resumption state.** For each of spec-00 … spec-06, classify into one of five states (see [ADOPTION-PROTOCOL.md](./ADOPTION-PROTOCOL.md) §1 for the full decision table):
   ...
   - `gate-passed` — file ends with `<!-- gate-result: PASS date=YYYY-MM-DD reviewer=dm-spec-creation -->`.
   ...
   if FAIL:
      ...
      declare DONE to user, AND hand off:
         "All specs and the cross-spec gate report PASS.
          Implementation is owned by the sibling `dm-spec-execution` skill,
          ...
          Invoke `dm-spec-execution` to begin implementation."
```
...
4. Append the result block to the bottom of the spec file:
   ...
   ```markdown
   <!-- gate-result: PASS date=2026-05-31 reviewer=dm-spec-creation -->
   ```

   or:

   ```markdown
   <!-- gate-result: FAIL date=2026-05-31 reviewer=dm-spec-creation
   findings:
