...
Reusable gate. Runs once per emitted sidecar file matching `01-specifications/spec-02[a-z]-*.md`. Invoked by the dm-spec-creation orchestrator inside the spec-02 family loop, before the cross-spec consistency gate.
...
```markdown
<!-- gate-result: PASS date=YYYY-MM-DD reviewer=dm-spec-creation/spec-02x-sidecar-gate -->
```

On FAIL, stamp:

```markdown
<!-- gate-result: FAIL date=YYYY-MM-DD reviewer=dm-spec-creation/spec-02x-sidecar-gate
findings:
