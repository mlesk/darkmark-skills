---
name: spec-06-execution-plan
description: Execution-plan grilling workflow for dm-spec-creation spec-06. Agent-only sub-skill invoked by the dm-spec-creation orchestrator to produce the agent-executable inside-out Phase plan consumed by dm-spec-execution. Do not invoke directly; invoke dm-spec-creation instead.
...
20. Reference the canonical execution-state schema in planning standard §10 (do not duplicate it). State that the file lives at `01-specifications/execution-state.md` and is created on first execution by the sibling `dm-spec-execution` skill per planning standard §11 step 0.
...
22. Reference planning standard §11 (and §11a failure protocol + §11b context-budget halt protocol) as the canonical state machine; do not duplicate. Note that the sibling `dm-spec-execution` skill is the orchestration entry point.
