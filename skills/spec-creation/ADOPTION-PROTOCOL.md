...
How `spec-creation` picks up a set of **pre-existing specs** that were not authored through this skill, evaluates them, fills gaps, and runs them through the same gates as freshly-authored specs.
...
```text
[spec-creation] spec-NN adopted: <X anchors injected> | <Y missing sections scaffolded> | <Z gate questions asked> | gate PASS
```
