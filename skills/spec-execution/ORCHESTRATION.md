# Orchestration Protocol — spec-execution-pocockified

The state machine lives in `.github/skills/spec-creation/standards/planning-standards-inside-out-phases.md` §11. This file defines the **bootstrap** and the **error envelopes** around that loop. Do not duplicate the loop here.

## 1. Bootstrap (run every invocation)

1. **Verify the sibling skill exists.** The following paths MUST resolve:
   - `.github/skills/spec-creation/standards/planning-standards-inside-out-phases.md`
   - `.github/skills/spec-creation/standards/coding-standards-testing.md`
   - `.github/skills/spec-creation/STANDARDS-PROTOCOL.md`

   If any are missing, halt: `sibling skill 'spec-creation' is missing or corrupt; restore it before invoking spec-execution-pocockified.`

2. **Verify spec-creation reported DONE.** Open `01-specifications/decisions.md` and confirm a `<!-- cross-spec-gate: PASS … -->` block exists at the top. If absent, halt: `cross-spec gate has not passed; run spec-creation to completion before invoking spec-execution-pocockified.`

3. **Locate the plan.** Open `01-specifications/spec-06-execution-plan.md`. If missing or its gate-result block is not `PASS`, halt: `spec-06 has not passed its gate; run spec-creation first.`
...
- it MUST end with `<!-- gate-result: PASS … reviewer=spec-creation/spec-02x-sidecar-gate -->`;
- it MUST contain an inline `<a id="<anchor-id>"></a>` for every cited anchor.

  On any failure, halt: `SPEC-DEFECT — sidecar <file> missing, failed, or anchor <anchor-id> not present; run spec-creation first.` Do not attempt to repair.
...
| Spec defect found                 | `blocked-by: spec-defect-...`           | `SPEC-DEFECT — <P-id>: <issue>` (run spec-creation)         |
| Sidecar defect found              | `blocked-by: spec-defect-sidecar-...`   | `SPEC-DEFECT — sidecar <file>: <issue>` (run spec-creation) |
...
- Never re-run `spec-creation` from inside this skill.
