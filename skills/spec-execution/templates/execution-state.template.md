...
> Canonical schema lives in `.github/skills/spec-creation/standards/planning-standards-inside-out-phases.md` §10. This template is the **initial** file content the agent writes during planning standard §11 step 0. Fill `<plan-sha>`, `<sidecar-shas>`, and the per-Phase blocks from `spec-06-execution-plan.md`.
...
The file is written by `spec-execution-pocockified` only. Plan authors do not hand-edit it.
