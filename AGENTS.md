...
- Registration is symlink-based: `scripts/link-skills.sh` links every user-invocable `SKILL.md` under `skills/` (except `deprecated/` and the agent-only `dm-spec-creation/specs/` sub-skills) into `~/.agents/skills/`. `scripts/unlink-skills.sh` removes links that point into this repo.
